...
pub fn ping_back(destination_ip: Ipv4Addr, on_processed: Option<Arc<Box<dyn Fn(String) + Send + Sync>>>, echo_replies:Arc<Mutex<HashMap<String,bool>>>) -> Result<String, String> {
    ...
    let sockaddr_in = SOCKADDR_IN {
     ...
     let payload = create_icmp_echo_request(0x1234, 1, b"ping test");
        ...
        if received == SOCKET_ERROR {
           ...
           return Err("Timeout or error".to_string());
        }else {
            //let elapsed = start.elapsed();
            //println!("recv_buf {:#?}",recv_buf);
            
            let from_ip = Ipv4Addr::from(u32::from_be(unsafe { from.sin_addr.S_un.S_addr }));
            println!("to {} -- from {}",destination_ip,from_ip);


            if destination_ip != from_ip{

                return Err("Response IP".to_string());
            }
            let icmp_offset = 20; // IP header size (sin opciones)
            let icmp_type = recv_buf[icmp_offset];
            let icmp_code = recv_buf[icmp_offset + 1];
            ...
            if icmp_type == 0 && icmp_code == 0 {
                  println!(
                    "{} - Respuesta recibida en {} ms (ICMP Type: {}, Code: {})",
                    destination_ip,0, //elapsed.as_millis(),
                    icmp_type,
                    icmp_code
                  );
            ...
            Ok(destination_ip.to_string())
                  ...
                  println!(
                   "{} - Paquete recibido pero no es Echo Reply (Type: {}, Code: {})",
                    destination_ip,
                    icmp_type,
                    icmp_code
                  );

                return Err("Error icmp".to_string());
            }
        }
